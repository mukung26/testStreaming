...
export function Home() {
  ...
  const currentType = typeParam === 'tv' ? 'tv' : 'movie';
  ...
  return (
    <div className="flex flex-col gap-6 bg-[#0A0A0B]">
      {/* Hero Section */}
      <div className="relative h-[450px] md:h-[550px] flex-shrink-0">
        {heroItem && (
          <>
            <img 
            ...
            <div className="relative z-20 h-full flex flex-col justify-center px-8 md:px-12 gap-4 max-w-3xl">
              <div className="flex items-center gap-2 text-blue-400 font-bold text-xs uppercase tracking-[0.3em]">
                ...
                <span>• {currentType === 'movie' ? 'Movie' : 'TV Series'}</span>
                <span className="text-yellow-500 bg-yellow-500/10 px-2 py-0.5 ml-2 rounded-sm border border-yellow-500/20 text-[10px]">★ {heroItem.vote_average?.toFixed(1)}</span>
              </div>
              <h1 className="text-4xl md:text-6xl font-black uppercase leading-tight text-white drop-shadow-lg">
                {heroItem.title || heroItem.name}
              </h1>
              <p className="text-gray-300 max-w-xl leading-relaxed text-sm hidden sm:block drop-shadow-md line-clamp-3">
                {heroItem.overview}
              </p>
              <div className="flex gap-4 mt-4">
                <Link to={`/details/${currentType}/${heroItem.id}`} className="bg-white text-black px-6 md:px-8 py-3 rounded-sm font-bold uppercase text-xs flex items-center gap-2 hover:bg-blue-500 hover:text-white transition-all">
      ...
      <div className="p-8 pt-2 flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold uppercase tracking-widest flex items-center gap-3 text-white">
            ...
            Discover All {currentType === 'movie' ? 'Movies' : 'TV Shows'}
          </h2>
        </div>
